...
public class InfoAction extends InfoSuperAction {	
	public String ListShow(){
		...
		if(currentP==1){     		//设置显示第1页信息的SQL语句
			...
			sqlFreeSub="SELECT * FROM tb_info i WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') AND (info_date < (SELECT MIN(info_date) FROM (SELECT  (info_date) FROM tb_info WHERE (info_type = i.info_type) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC LIMIT "+top2+") AS mindate)) ORDER BY info_date DESC LIMIT "+top1+" ";
//			sqlFreeSub="SELECT * FROM tb_info i WHERE (info_type = ?) AND (info_state = '1') AND (info_payfor = '0') AND (info_date NOT IN (SELECT info_date FROM tb_info WHERE (info_type = i.info_type) AND (info_state = '1') AND (info_payfor = '0') ORDER BY info_date DESC LIMIT "+top2+")) ORDER BY info_date DESC LIMIT "+top1+"";         //另一种实现分页查询的SQL语句
		}
	...
	public String Add(){
		...
		if(addType.equals("add")){
			...
			String sql="insert into tb_info values(id,?,?,?,?,?,?,?,?,?)";
...
//			sqlSearchSub="SELECT * FROM tb_info WHERE ("+subsql+opname+"?) AND (info_date NOT IN (SELECT info_date FROM tb_info WHERE "+subsql+opname+"'"+param+"' ORDER BY info_date DESC LIMIT "+top2+")) ORDER BY info_date DESC LIMIT "+top1+"";				//另一种实现分页查询的SQL语句
		}
