...
public class AdminAction extends AdminSuperAction {
	...
	public String ListShow() {
		...
		if (!stateType.equals("all") && !payforType.equals("all")) { // 没有同时选择“付费状态”与“审核状态”中的“全部”选项
			...
			sqlsub = "SELECT TOP "
					+ perR
					+ " * FROM tb_info WHERE (info_type=?) AND (info_state=?) AND (info_payfor=?) ORDER BY info_date DESC";
			params = new Object[3];
			...
			sqlsub = "SELECT * FROM tb_info WHERE (info_type=?) ORDER BY info_date DESC LIMIT "
					+ perR + "";
			params = new Object[1];
			params[0] = Integer.valueOf(infoType);
			...
			sqlsub = "SELECT * FROM tb_info WHERE (info_type=?) AND (info_state=?) ORDER BY info_date DESC LIMIT "
					+ perR + "";
			params = new Object[2];
			params[0] = Integer.valueOf(infoType);
			params[1] = stateType;
			...
			sqlsub = "SELECT * FROM tb_info WHERE (info_type=?) AND (info_payfor=?) ORDER BY info_date DESC LIMIT "
					+ perR + "";
			params = new Object[2];
			params[0] = Integer.valueOf(infoType);
			params[1] = payforType;
		...
		OpDB myOp = new OpDB();
		CreatePage createPage = myOp.OpCreatePage(sqlall, params, perR,
				strCurrentP, gowhich); // 调用OpDB类中的OpCreatePage()方法计算出总记录数、总页数，并且设置当前页码，这些信息都封装到了createPage对象中

		int currentP = createPage.getCurrentP();
		if (currentP > 1) {
			int top = (currentP - 1) * perR;
			if (mark.equals("1")) {
				sqlsub = "SELECT * FROM tb_info i WHERE (info_type = ?) AND (info_payfor = ?) AND (info_state = ?) AND (info_date < (SELECT MIN(info_date) FROM (SELECT  (info_date) FROM tb_info WHERE (info_type = i.info_type) AND (info_payfor = i.info_payfor) AND (info_state = i.info_state) ORDER BY info_date DESC LIMIT "
						+ top
						+ ") AS mindate)) ORDER BY info_date DESC LIMIT "
						+ perR + "";
			} else if (mark.equals("2")) {
				sqlsub = "SELECT * FROM tb_info i WHERE (info_type = ?) AND (info_date < (SELECT MIN(info_date) FROM (SELECT  (info_date) FROM tb_info WHERE (info_type = i.info_type) ORDER BY info_date DESC LIMIT "
						+ top
						+ " ) AS mindate)) ORDER BY info_date DESC LIMIT "
						+ perR + "";
			} else if (mark.equals("3")) {
				sqlsub = "SELECT * FROM tb_info i WHERE (info_type = ?) AND (info_state = ?) AND (info_date < (SELECT MIN(info_date) FROM (SELECT (info_date) FROM tb_info WHERE (info_type = i.info_type) AND (info_state = i.info_state) ORDER BY info_date DESC LIMIT "
						+ top
						+ ") AS mindate)) ORDER BY info_date DESC LIMIT "
						+ perR + "";
			} else if (mark.equals("4")) {
				sqlsub = "SELECT * FROM tb_info i WHERE (info_type = ?) AND (info_payfor = ?) AND (info_date < (SELECT MIN(info_date) FROM (SELECT (info_date) FROM tb_info WHERE (info_type = i.info_type) AND (info_payfor = i.info_payfor) ORDER BY info_date DESC LIMIT "
						+ top
						+ ") AS mindate)) ORDER BY info_date DESC LIMIT "
						+ perR + "";
			}
	...
	public String CheckShow() {
		...
		if (infoSingle == null) { // 信息不存在
			...
			addFieldError("AdminShowNoExist",
					getText("city.singleshow.no.exist"));
		}
		return SUCCESS;
	}
	...
	public String Check() {
		...
		if (i > 0) { // 审核信息成功
			...
			comebackState();
			addFieldError("AdminCheckUnSuccess",
					getText("city.admin.check.no.success"));
			request.setAttribute("mainPage", "/pages/error.jsp");
			return "UnSuccess";
		}
	}
	...
	public String Delete() {
		...
		if (i > 0) { // 删除信息成功
			...
			comebackState();
			addFieldError("AdminDeleteUnSuccess",
					getText("city.admin.delete.no.success"));
			request.setAttribute("mainPage", "/pages/error.jsp");
			return "UnSuccess";
		}
	}
	...
	public String SetMoney() {
		...
		int i = myOp.OpUpdate(sql, params);
		if (i > 0) { // 信息付费设置成功
			addFieldError("AdminSetMoneySuccess",
					getText("city.admin.setmoney.success"));
			request.setAttribute("mainPage", "/pages/error.jsp");
			return "setMoneySuccess";
		} else { // 信息付费设置失败
			addFieldError("AdminSetMoneyUnSuccess",
					getText("city.admin.setmoney.no.success"));
			request.setAttribute("mainPage", "/pages/error.jsp");
			return "UnSuccess";
		}
	}
	...
	public void validateListShow() {
		...
		if (adminOP.equals("Check") || adminOP.equals("Delete")) // 如果是进行了“通过审核”或“删除信息”操作后，再来调用ListShow()方法显示信息列表，则需要恢复之前选择的“显示方式”和“信息类别”状态，从而重新在ListShow()方法中查询出符合条件的记录
			...
			if (getInfoType <= 0) { // 没有选择"信息类别”
				addFieldError("AdminListNoType",
						getText("city.admin.list.no.infoType"));
			} else {
				if (getPayforType == null || getPayforType.equals("")) { // 没有选择"付费状态”选项
					addFieldError("AdminListNoPayForType",
							getText("city.admin.list.no.payforType"));
				}
				if (getStateType == null || getStateType.equals("")) { // 没有选择"审核状态”选项
					addFieldError("AdminListNoStateType",
							getText("city.admin.list.no.stateType"));
				}
			}
		}
	}
	...
	public void validateSetMoneyShow() {
		...
		if (moneyID == null || moneyID.equals("")) { // 没有输入信息ID值
			addFieldError("moneyIDError",
					getText("city.admin.setMoney.no.moneyID"));
		} else { // 验证输入的信息ID值是否为数字格式
			try {
				int id = Integer.parseInt(moneyID);
				if (id < 0) // 若ID为负数
					addFieldError("moneyIDError",
							getText("city.admin.setMoney.moneyID.0"));
			} catch (NumberFormatException e) {
				addFieldError("moneyIDError",
						getText("city.admin.setMoney.format.moneyID"));
				e.printStackTrace();
			}
		}
	}
	...
	private void comebackState() {
		/*
		 * 获取session中保存的选择状态。 将选择状态保存在session中， 是在管理员单击"显示"按钮请求列表显示时，
		 * 在ListShow()方法中实现的
		 */
		...
		/* 恢复选择的状态 */
		if (getPayForType != null && getStateType != null
				&& getInfoType != null) {
